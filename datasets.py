...
class GetMRI(Dataset):
    ...
    def get_unet_input(self, list, length=512, low_detector=32):
        low_data = np.zeros((low_detector, length))
        distance = length // low_detector
        for i in range(low_detector):
            low_data[i, :] = list[i * distance, :]
...
#siat_input = loadmat(self.data_names[index])[list(loadmat(self.data_names[index]).keys())[-1]].astype(np.float32)
        siat_input = cv2.imread(self.data_names[index], cv2.IMREAD_GRAYSCALE).astype(np.float32)/255.
        tmep_name_le= self.data_names[index].split("good")
        a = random.choice([1,2,4,6,8])
        tmep_name=tmep_name_le[0]+"f"+str(a)+tmep_name_le[1]
        #tmep_name = tmep_name.replace(".jpg", ".png")
#siat_input_37BW = loadmat(tmep_name)[list(loadmat(tmep_name).keys())[-1]].astype(np.float32)
        siat_input_50BW = cv2.imread(tmep_name, cv2.IMREAD_GRAYSCALE).astype(np.float32)/255.

        return (siat_input[np.newaxis, :], siat_input_50BW[np.newaxis, :])
    def __len__(self):
...
def get_dataset(config, uniform_dequantization=False, evaluation=False):
    ...
    dataset = GetMRI(
        root="/home/b110/code/LLX/lijiao_1_17/dataset/xunlian/xunlian/good/"
    )
    test_dataset = GetMRI(
        root="/home/b110/code/LLX/lijiao_1_17/dataset/xunlian/xunlian/good/"
    )
    ...
    train_ds = DataLoader(
        dataset, batch_size=config.training.batch_size, shuffle=True, num_workers=0
    )
